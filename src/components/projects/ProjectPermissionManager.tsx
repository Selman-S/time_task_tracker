...
export default function ProjectPermissionManager({ 
  projectId, 
  projectName, 
  brandName,
  permissions, 
  ...
  const handleAddPermission = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/projects/${projectId}/permissions`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
  ...
  const handleUpdatePermission = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/projects/${projectId}/permissions/${editingPermission.user.id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const handleRemovePermission = async (permission: Permission) => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/projects/${projectId}/permissions/${permission.user.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-600">{brandName} → {projectName}</p>
        </div>
        <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Add a user to {projectName} with specific permissions.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
        ...
        <Card className="border-0 shadow-md bg-gray-50/50">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-muted-foreground">
              Add users to this project to start collaborating.
            </p>
          </CardContent>
        </Card>
      ) : (
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            Are you sure you want to remove {permission.user.name} from {projectName}? 
